...
status_t uartMgr_t::txPoll(uint8_t* data, uint32_t size)
{
	...
	pptFlag |= txBusy;
	LPUART_WriteBlocking(base, data, size);
	pptFlag &= (~txBusy);
...
status_t uartMgr_t::rxPoll(uint8_t* data, uint32_t size)
{
	...
	pptFlag |= rxBusy;
	LPUART_ReadBlocking(base, data, size);
	pptFlag &= (~rxBusy);
	return kStatus_Success;
}

status_t uartMgr_t::txIntr(uint8_t* data, uint32_t size, lpuart_transfer_callback_t callback, void* userData)
{
	if (pptFlag & txBusy) { return kStatus_Fail; }
	pptFlag |= txBusy;
	LPUART_TransferCreateHandle(base, &txIntrHandle, callback, userData);
	static lpuart_transfer_t xfer;
	xfer.data = data;
	xfer.dataSize = size;
	return LPUART_TransferSendNonBlocking(base, &txIntrHandle, &xfer);
}

void uartMgr_t::defaultCallback(LPUART_Type* base, lpuart_handle_t* handle, status_t status, void* userData)
{
...
void uartMgr_t::txIntrAbort(void)
{
	if (pptFlag & txBusy)
	{
		LPUART_TransferAbortSend(base, &txIntrHandle);
		pptFlag &= (~txBusy);
	}
}

status_t uartMgr_t::rxIntr(uint8_t* data, uint32_t size, lpuart_transfer_callback_t callback, void* userData)
{
	if (pptFlag & rxBusy) { return kStatus_Fail; }
	pptFlag |= rxBusy;
	LPUART_TransferCreateHandle(base, &rxIntrHandle, callback, userData);
	static lpuart_transfer_t xfer;
	xfer.data = data;
	xfer.dataSize = size;
	return LPUART_TransferReceiveNonBlocking(base, &rxIntrHandle, &xfer, (size_t*)& rxIntrCnt);
}
...
void uartMgr_t::rxIntrAbort(void)
{
	if (pptFlag & rxBusy)
	{
		LPUART_TransferAbortReceive(base, &rxIntrHandle);
		pptFlag &= (~rxBusy);
	}
}
...
void uartMgr_t::tp_defaultcallback(LPUART_Type* base, lpuart_handle_t* handle, status_t status, void* userData)
{
